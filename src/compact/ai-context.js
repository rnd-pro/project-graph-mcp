...
export async function getAiContext(a,l={}){const{includeFiles:f=[],includeDocs:m=!0,includeSkeleton:d=!0}=l,p=e(a),u={};const allJS=r(p);const wantAll=f.includes("*");
let g=0;if(!wantAll&&d&&(u.skeleton=await s(p),g+=estimateTokens(u.skeleton)),!wantAll&&m){const e=await o(p);u.docs=n(e,p),g+=estimateTokens(u.docs)}if(wantAll||f.length>0){u.files={};const ignorePatterns=wantAll?_loadIgnore(p):[];
