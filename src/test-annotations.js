...
const s=t.match(/^- \[([ x!])\] (\w+):\s*(.+)$/);if(!s)continue;const[,a,c,i]=s,f=i.split("→").map(t=>t.trim()),u=f[0],p=f[1]||null;
let l=null,d="pending";if("x"===a&&(d="passed"),"!"===a){d="failed";
const t=u.match(/\(FAILED:\s*(.+)\)$/);t&&(l=t[1].trim())}r.push({name:c,action:u,expected:p,status:d,failReason:l})}return o&&r.length&&n.push(...groupByName(r,e)),n}
...
function updateTestState(e,s,r,a){const c=process.cwd(),i=findCtxMdFiles(o(c,".context")),f=parseInt(s.split(".")[1],10);for(const o of i)try{const c=t(o,"utf-8").split("\n");
let i=!1,u=0;for(let t=0;t<c.length;t++){if(c[t].startsWith("## ")){i=c[t].startsWith("## Tests");continue}if(!i)continue;
const p=c[t].match(/^- \[([ x!])\] (\w+):\s*(.+)$/);if(p&&p[2]===e){if(u===f){const i=p[3].replace(/\s*\(FAILED:.*\)$/,""),f=a?` (FAILED: ${a})`:"";return c[t]=`- [${r}] ${e}: ${i}${f}`,n(o,c.join("\n"),"utf-8"),{success:!0,testId:s,...a?{reason:a}:{}}}u++}}}catch(t){}return{success:!1,testId:s,error:"Test not found"}}
