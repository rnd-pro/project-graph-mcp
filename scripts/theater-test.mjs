...
async function runTheaterTest() {
  ...
  const actions = [ {
    name: "Loading web skeleton",
    tool: "get_skeleton",
    args: {
      path: "web"
    }
  }, {
    name: "Tracing usages of followController",
    tool: "navigate",
    args: {
      action: "usages",
      symbol: "followController"
    }
  }, {
    name: "Compacting / Viewing app.js",
    tool: "compact",
    args: {
      action: "compact_file",
      path: "web/app.js"
    ...
    name: "Analyzing follow-controller.js",
    tool: "analyze",
    args: {
      action: "full_analysis",
      path: "web/follow-controller.js"
    }
  } ];
  for (const action of actions) {
    ...
    let waited = 0;
    while (receivedEvents.length === 0 && waited < 1e4) {
      await sleep(100);
      waited += 100;
    }
