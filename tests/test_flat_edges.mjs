...
for (const file of Object.keys(projectCtx.X || {})) {
  fileMap.set(file, "n_X_" + file);
}

for (const [dir, names] of Object.entries(projectCtx.f || {})) {
  for (const name of names) {
    fileMap.set(dir === "./" ? name : dir + name, "n_f_" + name);
  }
}
...
for (const [srcFile, sources] of Object.entries(projectCtx.I || {})) {
  ...
  for (const impPath of sources) {
    ...
    let targetFile = null;
    if (fileMap.has(impPath)) targetFile = impPath; else {
      let base = impPath.split("/").pop();
      for (const known of fileMap.keys()) {
        if (known.endsWith("/" + base) || known.endsWith("/" + base + ".js")) {
          targetFile = known;
          break;
        }
      }
