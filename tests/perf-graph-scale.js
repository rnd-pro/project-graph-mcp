...
function generateSkeleton(nodeCount, edgeDensity = .03) {
  const skeleton = {
    n: {},
    X: {},
    I: {},
  ...
  const files = [];
  const dirs = [ "src/", "src/core/", "src/utils/", "src/api/", "src/models/", "src/services/", "src/controllers/", "src/middleware/", "src/config/", "src/views/", "src/helpers/", "src/routes/", "src/db/", "src/auth/", "src/tests/" ];
...
function resolveImport(imp, from, known) {
  ...
  if (imp.startsWith(".")) {
    ...
    for (const p of parts) {
      if (p === "..") norm.pop(); else if (p !== ".") norm.push(p);
    }
    resolved = norm.join("/");
...
function buildFileGraph(skeleton) {
  ...
  for (const [dir, names] of Object.entries(skeleton.f || {})) for (const name of names) files.add(dir === "./" ? name : dir + name);
  for (const [dir, names] of Object.entries(skeleton.a || {})) for (const name of names) files.add(dir === "./" ? name : dir + name);
  ...
  for (const file of files) {
    const node = new Node(baseName(file), {
      type: "file",
      category: "file"
    });
    node.params = {
  ...
  for (const [srcFile, targets] of Object.entries(skeleton.I || {})) {
    ...
    for (const tgtFile of targets) {
      ...
      edgesAdded.add(key);
      try {
        editor.addConnection(new Connection(editor.getNode(srcId), "out", editor.getNode(tgtId), "in"));
      } catch {}
    }
  }
  const groups = {};
...
function testScale(nodeCount) {
  ...
  const {skeleton: skeleton} = gen.result;
  const build = measure(() => buildFileGraph(skeleton));
  const {editor: editor, groups: groups} = build.result;
  const nodes = editor.getNodes().length;
  const conns = editor.getConnections().length;
  const layout = measure(() => computeAutoLayout(editor, {
    groups: groups
  }));
  return {
    scale: nodeCount,
...
const scales = [ 100, 500, 1e3, 2e3, 3e3, 5e3 ];
...
for (const n of scales) {
  ...
  console.log(`${pad(r.scale, 6)} | ${pad(r.nodes)} | ${pad(r.conns)} | ${ms(r.generate)} | ${ms(r.buildGraph)} | ${ms(r.autoLayout)} | ${ms(r.total)}`);
}
...
for (let i = 1; i < results.length; i++) {
  ...
  const ratio = key => {
    const v = prev[key] > .1 ? (curr[key] / prev[key]).toFixed(1) : "—";
...
if (results.length >= 4) {
  ...
  for (const key of [ "buildGraph", "autoLayout" ]) {
    const timeRatio = r2[key] / Math.max(r1[key], .01);
